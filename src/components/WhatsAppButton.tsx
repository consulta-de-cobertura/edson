...
const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({ onOpenModal }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <button
        onClick={handleClick}
        className="group relative flex items-center bg-green-500 hover:bg-green-600 text-white rounded-full shadow-2xl transition-all duration-300 hover:scale-110 animate-whatsapp-pulse"
        aria-label="Entrar em contato via WhatsApp"
      >
        {/* Tooltip */}
        <div className="absolute right-full mr-4 bg-gray-800 text-white px-4 py-2 rounded-lg text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
